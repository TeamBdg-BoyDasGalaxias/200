...
const { bdgvip } = require('./src/bdgvip')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:bdg\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5511941791973:+55 1194179-1973\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\n Sejá bem vindo(a) ao grupo *${mdata.subject}*\n\nEspero que goste do grupo ❤️`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ƬΣΛMBDG 🐦🚓*',
					...
					userB: `──「 LISTA 」──\nOlá !\nDesculpe. Você não está registrado como amigo do BDG. Registre-se para fazer amizade com a ƬΣΛMBDG 🐦🚓 por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar BDG|17\n\n──「 ƬΣΛMBDG 🐦🚓 」──`,
			...
			const mod = [ownerNumber,"5511941791973@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5511941791973@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5511941791973@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5511941791973@s.whatsapp.net","554191232721@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
            ...
            if (messagesC.includes("dinheiro")){
            client.updatePresence(from, Presence.composing)
            reply("MEU PIX = donate@teambdg.ga")
    }

            if (messagesC.includes("cc")){
            client.updatePresence(from, Presence.composing)
            reply("MEU PIX = donate@teambdg.ga")
    }

                if (messagesC.includes("pix")){
            client.updatePresence(from, Presence.composing)
            reply("MEU PIX = donate@teambdg.ga")
    }
...
//@darkYT					
    ...
    break
				...
				case 'textblue':
					...
					break
					case 'vipmenu':
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *#bdgvip* para comprar acesso Premium!' ,text, { quoted: mek })
				...
				case 'premiumlist':
					...
					break
                  case 'qrcode':
        			if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *.bdgvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO ƬΣΛMBDG BOT 👑* 」\n\`\`\``
					no = 0
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando só para o meu dono, qm é vc arrombado?')
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
                ...
                if (!isGroup) return reply(mess.only.group)
                    ...
                    break
			    case 'owner':
                    client.sendMessage(from, {displayname: "BDG", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'walpaperanime':
                    ...
                    break
					case 'bdgvip': 
					client.sendMessage(from, bdgvip(prefix) , text, { quoted: mek })
					break
					case 'nekopoi':   
				...
				case 'neko':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT BDG ✨* 」\n*│+ wa.me/5511941791973*╰──────*「 *ƬΣΛMBDG 🐦🚓* 」*────*\n\n*_SE QUER SER ADMIN DA ƬΣΛMBDG_*\n*_Tipo /iklan_*' })
                    break
                case 'iklan':
                ...
                break
                    case 'modbotlist':
					teks = 'Esta é a minha lista de usuários premium :\n'
					for (let p of mod) {
						teks += `~> @${p.split('@')[0]}\n`
					}
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal da teambdg:*\n\n https://www.youtube.com/watch?v=9UFDBgQwzQc'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
                    memein = await kagApi.memeindo()
                    buffer = await getBuffer(`https://imgur.com/a/z9IhWqQ`)
                    client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* ƬΣΛMBDG 🐦🚓 \n*YOUTUBE:* https://www.youtube.com/channel/UCT-4ZXwauF23ftbbPQRnH6Q\n*WPP:* wa.me/+5511941791973\n*INSTA:* @_teambdg_\n\n\nEspero que tenham gostado do bot '})
                    break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
